...
int message_responder( int sock, message *msg_in ) {
	...
	if ( game_found ) set_game(game); // free lock of this game_node
    return stat;
}
...
void *read_data( void *arg ){
    ...
    printf("[%s:%s] connection recieved\n", host, port);
    ...
    if (curr->bytes == 0){
	printf("[%s:%s] got EOF\n", host, port);
	printf("[%s:%s] starting termination process\n", host, port);
    } else if (curr->bytes == -1){
	printf("[%s:%s] read error recieved: %s\n", host, port, strerror(error));
	printf("[%s:%s] starting termination process\n", host, port);
    } else{
	printf("[%s:%s] program error recieved: %s\n", host, port, get_msg_info_str(msg_info));
	printf("[%s:%s] starting termination process\n", host, port);
    }
	...
	close(con->fd);
	if (DEBUG) printf("closing partner socket: %d\n", sock_partner);
	close(sock_partner);

	// update socket status to inactive 
	if (DEBUG) printf("set is_active[%d] to false!\n", con->fd);
	...
	is_active[con->fd][1] = 0;
	waiting_game = NULL;
	if (DEBUG) printf("set waiting_game to NULL!\n");
    }

    // remove game if connection terminated randomly (not from game ending)
    if ( !remove_existing_game( con->fd ) ) { 
	if (DEBUG) printf("successfully removed game associated with sock %d!\n", con->fd);
    }
    else { 
	if (DEBUG) printf("no game associated with sock %d to remove!\n", con->fd);
    }

    // remove sock_buf_mem
    if ( !rm_sock_buf_node( con->fd ) ) {
	if (DEBUG) printf("successfully removed sock_buf_node associated with sock %d!\n", con->fd);
    }
    else {
	if (DEBUG) printf("no sock_buf_node associated with sock %d to remove!\n", con->fd);
    }

    free(con);
    
    printf("[%s:%s] data cleanup finished; termination process successful!\n", host, port);
...
int main(int argc, char ** argv){
	...
	if (DEBUG) printf("accepted socket %d\n", con->fd);
	...
	if (DEBUG) printf("set is_active[%d] to true!\n", con->fd);
